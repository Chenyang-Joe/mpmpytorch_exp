...
def set_boundary_conditions(model: MPMSolver, bc_params: DictConfig):
    ...
    for bc in bc_params:
        ...
        elif bc["type"] == "sdf_collider_test":
            assert "start_time" in bc.keys()
            assert "end_time" in bc.keys()
            
            add_sdf_collider_test(
                model,
                start_time=bc["start_time"],
                end_time=bc["end_time"],
            )
...
def add_sdf_collider_test(
    model: MPMSolver,
    start_time: float=0.0,
    end_time: float=999.0,
):
    def sdf_xy2z(xyz, center=torch.tensor([0.5, 0.5], device=model.device)):
        xy = xyz[:, :2] - center  # (N, 2)
        dist = torch.sqrt((xy ** 2).sum(dim=1))  # (N,)
        a = -20
        b = 0.3
        height = a * dist**2 + b  # scalar field z=...

        choice = height < 0.1
        height[choice] = 0.1
        return height  # (N,)

    def sdf_xz2y(xyz, center=torch.tensor([0.5, 0.5], device=model.device)):
        xz = xyz[:, [0,2]] - center  # (N, 2)
        dist = torch.sqrt((xz ** 2).sum(dim=1))  # (N,)
        a = -2
        b = 0.3
        height = a * dist**2 + b  # scalar field z=...
